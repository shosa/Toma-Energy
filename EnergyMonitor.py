...
def open_settings():
    ...
    alarm_toggle_btn = tk.Button(settings_window, text="Attivo" if ALARM_ENABLED else "Disattivo",
                                 bg="green" if ALARM_ENABLED else "red", command=toggle_alarm)
    ...
    while True:
        try:
            ...
            if ALARM_ENABLED and not alarm_active:
                trigger_alarm()  # Attiva l'allarme anche in caso di errore di comunicazione
        time.sleep(3)
